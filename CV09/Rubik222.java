public class Rubik222 {
    private byte[][] top, down, left, right, front, back;
    private String solve;

    public Rubik222() {  // vytvori kocku s indexami 1.. 24
        byte c = 1;
        top = new byte[][]{{c, c}, {c, c}};
        c++;
        down = new byte[][]{{c, c}, {c, c}};
        c++;
        left = new byte[][]{{c, c}, {c, c}};
        c++;
        right = new byte[][]{{c, c}, {c, c}};
        c++;
        front = new byte[][]{{c, c}, {c, c}};
        c++;
        back = new byte[][]{{c, c}, {c, c}};
        c++;
        solve = "";
    }
    // cielom je alokovat co najmenej pamate, aby to nepadlo na GC
    // skopiruje kocku do novej a prida jeden elementarny tah
    ...
    // kopirovanie steny kocky
    public void assign(byte[][] to, byte[][] from) {
        for (int i = 0; i < 2; i++)
            for (int j = 0; j < 2; j++)
                to[i][j] = from[i][j];
    }
    // rotacia steny kocky, clockwise
    ...
    // rotacia kocky okolo osi x, anti-clockwise
    public void xPrime() {
        x();
        x();
        x();
    ...
    // predalokovane polia, aby sme nic nealokovali, co nemame pod kontrlolou
    ...
    //----------------------------------------- u
    public void u() {   // jednu elem rotaciu musime nakodit, vsetky ostatne potom dostaneme trikom
        restop[0][0] = top[1][0];
        restop[0][1] = top[0][0];
        restop[1][0] = top[1][1];
        restop[1][1] = top[0][1];
